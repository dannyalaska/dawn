...
def create_job(
    *,
    ...
    with session_scope() as session:
        ...
        fv = _resolve_feed_version(session, feed, feed_version)
...
) -> tuple[int, int, list[Any], dict[str, Any]]:
    if dry_run_report:
        dry = dry_run_report
        rows_in = int(dry.get("rows_before", 0))
        rows_out = int(dry.get("rows_after", 0))
        warnings: list[Any] = []
        removed = dry.get("columns_removed") or []
        if removed:
            warnings.append({"type": "columns_removed", "details": removed})
        validation = {"dry_run": dry}
    else:
        rows_in = 0
        rows_out = 0
        warnings = [{"type": "info", "message": "No dry-run data available; execution simulated."}]
        validation = {}
...
def run_job(job_id: int) -> dict[str, Any]:
    ...
    with session_scope() as session:
        ...
        dry_run_report = transform_version.dry_run_report if transform_version else None
        run = JobRun(job_id=job.id, status="running", started_at=started, logs=[])
        session.add(run)
        session.flush()
        run_id = run.id
    ...
    logs: list[dict[str, Any]] = [
        {"ts": started.isoformat(), "level": "info", "message": "Job started"}
    ]
    try:
        rows_in, rows_out, warnings, validation = _simulate_execution(dry_run_report)
        logs.append(
            {
        ...
        status = "failed"
        warnings.append({"type": "exception", "message": str(exc)})
        logs.append(
            {
                "ts": datetime.utcnow().isoformat(),
                "level": "error",
